<?php
namespace Spotii\Spotiipay\Block\Adminhtml\System\Config\Fieldset;
...
class Payment extends \Magento\Config\Block\System\Config\Form\Fieldset
{
    ...
    protected function _getHeaderTitleHtml($element)
    {
        ...
        $disabledAttributeString = $this->_isPaymentEnabled($element) ? '' : '';
        $disabledClassString = $this->_isPaymentEnabled($element) ? '' : '';
        $htmlId = $element->getHtmlId();
        $html .= '<div class="button-container"><button type="button"' .
            $disabledAttributeString .
            ' class="button action-configure' .
            (empty($groupConfig['spotii_ec_separate']) ? '' : ' spotii-ec-separate') .
            $disabledClassString .
            '" id="' .
            $htmlId .
            '-head" onclick="spotiiToggleSolution.call(this, \'' .
            $htmlId .
            "', '" .
            $this->getUrl(
                'adminhtml/*/state'
    ...
    protected function _getExtraJs($element)
    {
        $script = "require(['jquery', 'prototype'], function(jQuery){
            window.spotiiToggleSolution = function (id, url) {
                var doScroll = false;
